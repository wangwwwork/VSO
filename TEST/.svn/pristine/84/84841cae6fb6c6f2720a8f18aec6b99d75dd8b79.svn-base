...
var InHoursForm = React.createClass({
    ...
    getInitialState: function () {
        ...
        return { pageInputState: '',pageInputStateDescripe:'', inIterms, inputTotalTime: '',classId: '' };
    },
    ...
    storeSuccess: function () {
        var dialog = $('#dialog').data('dialog').close();
        React.render(<MainForm />, document.body);
    },
    ...
    storeDocToDataBase: function (){
        ...
        if (this.judgeTheCommit()){
            var _self = this;
            _self.setState({pageInputStateDescripe: '正在存档，请耐心等待....'});
            var dialog = $('#dialog').data('dialog');
            dialog.open();
            $.ajax({
                url:globeInsertManpower,
                type: 'POST',
                data: { "TimePeopleNumber": JSON.stringify(_self.state.inIterms.toString()), "userid": getUserId, "groupid": _self.state.classId},
                dataType: "json",
                success: function (data) {
                    _self.setState({pageInputStateDescripe: '数据录入成功,页面跳转中'});
                    setTimeout("$('#dialog').data('dialog').close();returnMainForm()", 1000);
                    //录入成功之后的操作
    ...
    render: function () {
        ...
        return (<div className="grid">
            ...
            <div data-role="dialog" id="dialog" className="padding20" data-close-button="false" data-overlay="true" data-overlay-color="op-dark" data-overlay-click-close="false">
                <h2>{this.state.pageInputStateDescripe}</h2>
            </div>
        </div>);
    }
});